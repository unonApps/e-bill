...
{
    /// <inheritdoc />
    public partial class FixConsolidateStoredProcForeignKey : Migration
    {
        /// <inheritdoc />
        protected override void Up(MigrationBuilder migrationBuilder)
        {
            // Drop the old stored procedure that causes FK violation on ResponsibleIndexNumber
            migrationBuilder.Sql("DROP PROCEDURE IF EXISTS sp_ConsolidateCallLogBatch");

            // Recreate with LEFT JOIN to EbillUsers to validate ResponsibleIndexNumber
            // before inserting. If IndexNumber doesn't exist in EbillUsers, NULL is used
            // (ResponsibleIndexNumber is nullable).
...
BEGIN
    ...
    BEGIN TRY
        ...
        LEFT JOIN EbillUsers eu ON eu.IndexNumber = ISNULL(up.IndexNumber, s.IndexNumber)
        ...
        LEFT JOIN EbillUsers eu ON eu.IndexNumber = ISNULL(up.IndexNumber, a.IndexNumber)
        ...
        LEFT JOIN EbillUsers eu ON eu.IndexNumber = ISNULL(up.IndexNumber, p.IndexNumber)
        ...
        LEFT JOIN EbillUsers eu ON eu.IndexNumber = ISNULL(up.IndexNumber, pw.IndexNumber)
