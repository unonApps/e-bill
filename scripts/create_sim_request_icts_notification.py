...
<head>
    ...
    <title>New SIM Request - ICTS Action Required</title>
</head>
                    ...
                    <tr>
                        <td style="padding: 40px 30px;">
                            <p style="margin: 0 0 20px 0; color: #1f2937; font-size: 16px; line-height: 1.6;">
                                Dear <strong>ICTS Team</strong>,
                            </p>

                            <p style="margin: 0 0 30px 0; color: #4b5563; font-size: 15px; line-height: 1.6;">
                                A SIM card request has been approved by the supervisor and <strong style="color: #f59e0b;">requires ICTS processing</strong>. Please review and process this request at your earliest convenience.
                            </p>
                            ...
                            <div style="background-color: #dbeafe; border-left: 4px solid #009edb; padding: 15px 20px; margin-bottom: 30px; border-radius: 8px;">
                                ...
                                <p style="margin: 5px 0; color: #1e40af; font-size: 13px;">
                                    <strong>Supervisor Approved:</strong> {{SupervisorApprovalDate}}
                                </p>
                                <p style="margin: 5px 0 0 0; color: #1e40af; font-size: 13px;">
                                    <strong>Priority:</strong> <span style="background-color: #fef3c7; color: #92400e; padding: 2px 8px; border-radius: 4px; font-weight: 600;">Normal</span>
                                </p>
                            </div>
                            ...
                            <table width="100%" cellpadding="0" cellspacing="0" style="margin-bottom: 30px;">
                                <tr>
                                    <td colspan="2" style="background-color: #d1fae5; padding: 12px 20px; border-radius: 8px 8px 0 0; border-bottom: 2px solid #10b981;">
                                        <h2 style="margin: 0; color: #065f46; font-size: 18px; font-weight: 700;">SIM Card Details</h2>
                                    </td>
                                </tr>
                                <tr>
                                    <td style="padding: 15px 20px; border-bottom: 1px solid #e5e7eb; width: 40%;">
                            ...
                            <div style="background-color: #fef3c7; border-left: 4px solid #f59e0b; padding: 20px; margin-bottom: 30px; border-radius: 8px;">
                                <p style="margin: 0 0 8px 0; color: #92400e; font-size: 14px; font-weight: 600;">Additional Remarks:</p>
                                <p style="margin: 0; color: #78350f; font-size: 14px; line-height: 1.5;">{{Remarks}}</p>
                            </div>
                            {{/if}}

                            <!-- ICTS Action Items -->
                            <div style="background-color: #eff6ff; border-radius: 8px; padding: 20px; margin-bottom: 30px;">
                                <h3 style="margin: 0 0 15px 0; color: #1e40af; font-size: 16px; font-weight: 700;">ICTS Action Items</h3>
                                <ol style="margin: 0; padding-left: 20px; color: #1e40af; font-size: 14px; line-height: 1.8;">
                                    <li style="margin-bottom: 10px;"><strong style="color: #1e40af;">Verify Request:</strong> Review all requester information and request details.</li>
                                    <li style="margin-bottom: 10px;"><strong style="color: #1e40af;">Check Eligibility:</strong> Confirm that the requester is eligible for the requested SIM type.</li>
                                    <li style="margin-bottom: 10px;"><strong style="color: #1e40af;">Coordinate with Provider:</strong> Contact {{ServiceProvider}} to arrange SIM card procurement.</li>
                                    <li style="margin-bottom: 10px;"><strong style="color: #1e40af;">Update Status:</strong> Update the request status in the system after processing.</li>
                                    <li><strong style="color: #1e40af;">Notify Admin:</strong> Forward to admin team for final approval once processed.</li>
                                </ol>
                            </div>
                            ...
                            <div style="background-color: #fef2f2; border-left: 4px solid #dc2626; padding: 20px; margin-bottom: 30px; border-radius: 8px;">
                                <p style="margin: 0; color: #991b1b; font-size: 14px; line-height: 1.6;">
                                    <strong>SLA Reminder:</strong> Please process this request within <strong>2 business days</strong> to maintain service level agreements. The requester and supervisor have been notified of expected timelines.
                                </p>
                            </div>

                            <!-- Call to Action Button -->
                            <div style="text-align: center; margin: 30px 0;">
                                <a href="{{ProcessRequestLink}}" style="display: inline-block; background-color: #009edb; color: #ffffff; text-decoration: none; padding: 16px 40px; border-radius: 8px; font-size: 16px; font-weight: 700; box-shadow: 0 4px 12px rgba(0, 158, 219, 0.3); transition: all 0.3s ease;">
                                    Process Request →
                                </a>
                            </div>
...
sql_script = f"""-- Update SIM_REQUEST_ICTS_NOTIFICATION template
SET NOCOUNT ON;
GO

PRINT 'Updating SIM_REQUEST_ICTS_NOTIFICATION...';

UPDATE EmailTemplates
SET HtmlBody = '{html_escaped}',
    ModifiedDate = GETDATE()
WHERE TemplateCode = 'SIM_REQUEST_ICTS_NOTIFICATION';

IF @@ROWCOUNT > 0
    PRINT 'SIM_REQUEST_ICTS_NOTIFICATION updated successfully.';
ELSE
    PRINT 'WARNING: SIM_REQUEST_ICTS_NOTIFICATION not found or not updated.';
GO
...
FROM EmailTemplates WHERE TemplateCode = 'SIM_REQUEST_ICTS_NOTIFICATION';
GO
"""

# Write to file
with open('UPDATE_SIM_REQUEST_ICTS_NOTIFICATION.sql', 'w', encoding='utf-8') as f:
    f.write(sql_script)

print("SQL script for SIM_REQUEST_ICTS_NOTIFICATION created successfully")
