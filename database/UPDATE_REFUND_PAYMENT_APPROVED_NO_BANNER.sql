-- Remove success banner from REFUND_PAYMENT_APPROVED template
SET NOCOUNT ON;
GO

PRINT 'Updating REFUND_PAYMENT_APPROVED (removing success banner)...';
...
IF @@ROWCOUNT > 0
    PRINT 'REFUND_PAYMENT_APPROVED updated successfully (banner removed).';
ELSE
...
SELECT
    ...
    CASE
        WHEN HtmlBody LIKE '%Payment Approved!%' THEN 'Has Banner'
        ELSE 'No Banner'
    END AS 'Banner Status'
FROM EmailTemplates
WHERE TemplateCode = 'REFUND_PAYMENT_APPROVED';

PRINT '';
PRINT '=== Update Complete ===';
PRINT 'Success banner removed from REFUND_PAYMENT_APPROVED template.';
