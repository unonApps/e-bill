...
{
    /// <summary>
    /// Private Wire (PW) Call Record
    /// Represents dedicated private wire telecommunications service records
    /// </summary>
    [Table("PrivateWires")]
    public class PrivateWire
    {
        ...
        public decimal? AmountUSD { get; set; } // Was: usd - Private Wire often billed in USD

        [Column(TypeName = "decimal(18,4)")]
        [Display(Name = "Amount (KSH)")]
        public decimal? AmountKSH { get; set; } // Calculated from USD using exchange rate
        ...
        // Level4Unit, OrganizationalUnit - get from EbillUser.SubOffice/Organization
        ...
        [NotMapped]
        public decimal TotalCostUSD => AmountUSD ?? 0;

        [NotMapped]
        public decimal TotalCostKSH => AmountKSH ?? ((AmountUSD ?? 0) * 150); // Use AmountKSH if available, otherwise approximate conversion
        ...
        [NotMapped]
        public string ServiceType => "Private Wire";
    }
}